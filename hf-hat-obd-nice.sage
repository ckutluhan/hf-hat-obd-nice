...
class NiceHeegaardDiagram():
    ...
    def sort_generators(self):
        ...
        self.QSpinC_structures=[] #list of rational spinc structures with non-empty sets of generators
        for qS in self.QSpinC: 
            if qS not in self.QSpinC_structures:
                (self.QSpinC_structures).append(qS)
    ...
    def compute_order(self):
        ...
        d_zero=self.d_zero
        d_one=self.d_one
        if EH in self.img_d_zero:
            print("Order is 0.")
        elif d_zero.row(0)==0:
            print("Order is infinite.")
        else:
            #First put self.d_zero in row echelon form
            M=d_zero.augment(identity_matrix(GF(2),len(self.cx0)))
            E=M.echelon_form() #row reduced echelon form
            T=E[0:,len(self.cx1):] #base change matrix for cx0 resulting from row operations
            d_zero_mod=T*d_zero #d_zero matrix with respect to the new basis for cx0
            d_one_mod=T*d_one #d_one matrix with respect to the new basis for cx0

            #Next pass to quotients to make induced d_one injective
            K0=d_one_mod.right_kernel() #kernel of d_one
            K0mat=matrix(GF(2),K0.basis()).transpose() #matrix with columns given by a basis for K0
            d_zero_K0=(d_zero_mod*K0mat).column_space() #d_zero(K0)
            d_zero_K0mat=matrix(GF(2),d_zero_K0.basis()).transpose() #matrix with columns given by a basis for d_zero(K0)
            Q=identity_matrix(len(self.cx0)) #matrix representing quotient by d_zero(K0)
            for i in list(d_zero_K0mat.pivot_rows()):
                Q[i,i]=0
            Qd_one=Q*d_one_mod #d_one composed with quotient by d_zero(K0)
            K1=Qd_one.right_kernel() 
            while K1!=K0: #iterate the previous process
                K0=K1
                K0mat=matrix(GF(2),K0.basis()).transpose()
                d_zero_K0=(d_zero_mod*K0mat).column_space()
                d_zero_K0mat=matrix(GF(2),d_zero_K0.basis()).transpose()
                Q=identity_matrix(len(self.cx0))
                for i in list(d_zero_K0mat.pivot_rows()):
                    Q[i,i]=0
                Qd_one=Q*d_one_mod
                K1=Qd_one.right_kernel()
            ...
            if K.dimension()==len(self.cx1):
                ...
                Kmat=matrix(GF(2),K.basis()).transpose()
                d_zero_K=(d_zero_mod*Kmat).column_space()
                d_zero_Kmat=matrix(GF(2),d_zero_K.basis()).transpose()
                P=identity_matrix(len(self.cx1))
                Pred=P.delete_columns(Kmat.pivot_rows())
                Q=identity_matrix(len(self.cx0))
                Qred=Q.delete_rows(d_zero_Kmat.pivot_rows())
                d_zero_red=Qred*d_zero_mod*Pred #the map induced by d_zero from C1/K to C0/d_zero(K)
                d_one_red=Qred*d_one_mod*Pred #the map induced by d_one from C1/K to C0/d_zero(K)
                EHmat=matrix(GF(2),T*EH)
                EHmatred=EHmat.matrix_from_columns([j for j in range(len(self.cx0)) if j not in d_zero_Kmat.pivot_rows()])
                ...
                while U1!=U0:
                    ...
                    for v in U0.basis():
                        if delta*v in U0:
                            ...
                            continue
                    U1=U0.span(basis)
                U=U1 #the maximal subspace of C0 on which delta is defined  
                img_d_zero_red=d_zero_red.column_space()
                V=U.intersection(img_d_zero_red)
                orders=[]
                for v in V:
                    k=0
                    while delta^k*(EHred+v)!=0:
                        k+=1
                        if delta^k*(EHred+v)==0:
                            v_ord=k
                        else:
                            continue
                    orders.append(k)
